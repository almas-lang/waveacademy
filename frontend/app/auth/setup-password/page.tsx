...
function SetupPasswordForm() {
  ...
  useEffect(() => {
    if (!token) {
      setError('Invalid or missing setup token. Please contact your administrator for a new invitation link.');
    }
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authApi.setupPassword(token!, fields.password.value, fields.confirmPassword.value);
      setIsSuccess(true);
    } catch (error: any) {
